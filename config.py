...
# TRADING SETTINGS
# ===============================
SYMBOLS = ["BTCUSD", "ETHUSD"] if TESTNET else [
    s.strip() for s in os.getenv("SYMBOLS", "BTCUSD_PERP,ETHUSD_PERP").split(",")
]
...
# DEBUG STATUS OUTPUT
# ===============================
print(f"DEBUG: API_KEY PRESENT: {'YES' if API_KEY else 'NO'}")
print(f"DEBUG: API_SECRET PRESENT: {'YES' if API_SECRET else 'NO'}")
print(f"DEBUG: TELEGRAM_TOKEN PRESENT: {'YES' if TELEGRAM_TOKEN else 'NO'}")
print(f"DEBUG: TELEGRAM_CHAT_ID PRESENT: {'YES' if TELEGRAM_CHAT_ID else 'NO'}")
print(f"DEBUG: TESTNET MODE: {TESTNET}")
print(f"DEBUG: BASE_URL: {BASE_URL}")
print(f"DEBUG: SYMBOLS: {SYMBOLS}")
print(f"DEBUG: CAPITAL: ₹{CAPITAL}")
print(f"DEBUG: Risk: {RISK_PERCENT*100}% | SL: {SL_PERCENT*100}% | TP: {TP_PERCENT*100}% | RR: {RR_RATIO}")
print(f"DEBUG: MAX_TRADES: {MAX_TRADES}")
print(f"DEBUG: SUMMARY_FILE_PATH: {SUMMARY_FILE_PATH}")

# ===============================
# REQUIRED VARIABLE CHECK
# ===============================
required_vars = ["DELTA_API_KEY", "DELTA_API_SECRET", "TELEGRAM_TOKEN", "TELEGRAM_CHAT_ID"]
